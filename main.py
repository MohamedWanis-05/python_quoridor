...
from game.constants import FPS, TILE_SIZE
...
_AI_DISPATCH = {
    "Easy":   get_easy_ai_move,
    "Medium": get_medium_ai_move,
    "Hard":   get_hard_ai_move,
}
    ...
    for event in pygame.event.get():
        ...
        if game_state == "MENU":
            ...
            if selected_start_mode is not None:
                ...
                ai_difficulty  = selected_start_mode.get("difficulty", "Easy")
                game_state     = "PLAYING"
            ...
            if diagonal_position is not None:
                new_position = resolve_diagonal_move(board, player.player_id, diagonal_position)
            elif dr != 0 or dc != 0:
            ...
            if new_position not in {None, -100, -200}:
                board.set_player_position(player.player_id, new_position)
        ...
        if game_state == "PLAYING" and winner is None:
            ...
            if game_mode == "1vAI" and board.current_player.player_id == 2:
                ...
                ai_fn = _AI_DISPATCH.get(ai_difficulty, get_easy_ai_move)
                ai_move = ai_fn(board)
    ...
    if game_state == "MENU":
        ...
        if winner == 1:
            ...
            renderer.draw(board, p1_message="Wall block", p1_message_color=(255, 0, 0),
                          hovered_wall=hovered_wall, valid_moves=moves_to_draw)
